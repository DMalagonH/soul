   ...
   // pr($wp_query);
    ...
    if ($pages > 1) {
        ...
        if ($page > 1 && !empty($previouspage)) {
            $output .= "<a href='" . get_pagenum_link($page - 1) . "' class='prev tooltip' title='previous page'>$previouspage</a>";
        }
        ...
        if ($page < $pages && !empty($nextpage)) {
            $output .= "<a href='" . get_pagenum_link($page + 1) . "' class='next tooltip' title='next page'>$nextpage</a>";
        }
...
function wpthemess_paginate_loop($start, $max, $page = 0) {
    ...
    for ($i = $start; $i <= $max; $i++) {
        $output .= ($page === intval($i))
            ...
            : "<a href='" . get_pagenum_link($i) . "' class='pages tooltip' title='page $i'>$i</a>";
    }
    return $output;
}
?>