...
function wpt_get_featured_image($post_ID) {  
    $post_thumbnail_id = get_image_id_by_link ( get_post_meta($post_ID, 'webfoliop_pitemlink', true) );
...
function wpt_get_slide_image($post_ID) {  
    $post_thumbnail_id = get_image_id_by_link ( get_post_meta($post_ID, 'webfoliof_slideimage_src', true) );
    if ($post_thumbnail_id) {  
        $post_thumbnail_img = wp_get_attachment_image_src($post_thumbnail_id, 'small');  
        return $post_thumbnail_img[0];  
    }  
...
function wpt_slide_columns_content ( $column, $post_id ) {
    switch ( $column ) {
        ...
        case 'slider':
            ...
            break;
        case 'slide_caption':
            echo get_post_meta($post_id, 'webfoliof_fitemcaption', true);
            break;
    }  
} 
