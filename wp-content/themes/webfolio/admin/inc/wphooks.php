...
function site5framework_header_init() {
	...
	wp_enqueue_script( 'superfishjs', get_template_directory_uri().'/library/js/superfish.js');
    wp_enqueue_script( 'prettyphotojs', get_template_directory_uri().'/library/js/prettyphoto/jquery.prettyPhoto.js');
    wp_enqueue_script('tiptipjs', get_template_directory_uri().'/library/js/tiptip/jquery.tipTip.js');
...
function site5framework_custom_shortcut_favicon() {
	if (of_get_option('webfolio_custom_shortcut_favicon') != '') {
    echo '<link rel="shortcut icon" href="'. of_get_option('webfolio_custom_shortcut_favicon') .'" type="image/ico" />'."\n";
	}
...
*********************************************************************************************/
function site5framework_slider() {
	if (is_home() and of_get_option('webfolio_slidertype') != '' and of_get_option('webfolio_displayslider') != '') {
        if(of_get_option('webfolio_slidertype')=="nivo")  {
            wp_enqueue_script('nivo-slider', get_template_directory_uri().'/library/js/jquery.nivo.slider.pack.js');
            wp_enqueue_style('nivo-slider', get_template_directory_uri().'/library/css/nivo-slider.css');
        }        
        elseif(of_get_option('webfolio_slidertype')=="flex") {
...
add_action('wp_head', 'site5framework_slider');
...
function site5framework_portfolio1_init() {
	if (is_page_template('template.portfolio.php') &&  !is_admin()) {
...
function site5framework_analytics(){
	$output = of_get_option('webfolio_stats');
	if ( $output <> "" )
